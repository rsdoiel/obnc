...
Trees_Node Trees_NewBoolean(int b)
{
	...
	result->valueType = NO_VALUE;
	result->type = Trees_NewLeaf(TREES_BOOLEAN_TYPE);
	return result;
}
...
{
	assert(boolNode != NULL);
	assert(boolNode->valueType == NO_VALUE);
	assert((boolNode->symbol == TRUE) || (boolNode->symbol == FALSE));

	return boolNode->symbol == TRUE;
}
...
Trees_Node Trees_NewString(const char string[])
{
	...
	result->type = Trees_NewNode(TREES_STRING_TYPE, Trees_NewInteger((int) strlen(string)), NULL);
	return result;
}
